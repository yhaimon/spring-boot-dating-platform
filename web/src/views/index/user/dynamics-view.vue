<template>
  <div class="container">
    <h2>动态管理</h2>
    <!-- 引入动态发布组件，并监听事件 -->
    <dynamic-send @postSubmitted="fetchPosts" />
    <!-- 动态列表区域 -->
    <h2>我的动态</h2>
        ...
        <div class="post-box">
          ...
          <div class="user-info-container">
            ...
            <div class="user-details">
              ...
              <!-- <div class="post-time">发布：{{ post.createdAt }}</div> -->
              <div v-if="post.updatedAt" class="post-time">{{ post.updatedAt }}</div>
            </div>

            <!-- 添加更多按钮 -->
            <div class="more-button-container" @mouseenter="showMoreOptions = index"
          ...
          <div v-if="post.images" class="image-list">
            <img v-for="(img, imgIndex) in post.images" :key="imgIndex" :src="getPostImgUrl(img)" alt="动态图片"
              class="post-image" />
          </div> -->
          ...
          <div v-if="!isEditing[index] && post.images" class="image-list">
            <img v-for="(img, imgIndex) in post.images" :key="imgIndex" :src="getPostImgUrl(img)" alt="动态图片"
              class="post-image" @click="openImage(getPostImgUrl(img))" @mouseover="hoverImage(imgIndex)"
              @mouseout="resetHover" />
          </div>
...
import { listDynamicApi, dynamicListByIdApi, updateDynamicApi, createApi, likeDynamicApi, deleteDynamicApi } from '/@/api/dynamic';
...
import DynamicSend from './dynamic-send.vue'; // 引入动态发布组件
...
const fetchPosts = async () => {
  try {
    ...
    //const response = await listDynamicApi({});
    const response = await dynamicListByIdApi({ userId: userId });
    if (response.code === 200) {
      posts.value = await Promise.all(response.data.map(async (post, index) => {// 在这里添加 index 参数
        //posts.value = response.data.map( post => {
        if (post.images) {
          // 确保 post.images 是一个数组
          if (typeof post.images === 'string') {
            //或者//post.images = post.images.split(',').map(img => `${BASE_URL}/api/staticfiles/dynamic/${img}`);
        ...
        // 添加用户信息
        /*这种方式会出问题，头像获取到的是老旧的头像，！！！ 
        post.avatar = userStore?.avatar || AvatarIcon; // 默认头像
        post.user_name = userStore?.user_name || '匿名用户'; // 默认用户名 */

        // 调用 detailApi 获取用户信息
        const userResponse = await detailApi({ userId: userId });
        if (userResponse.code === 200 && userResponse.data) {
          post.avatar = userResponse.data.avatar || AvatarIcon; // 默认头像// 后端返回的字段
          post.user_name = userResponse.data.username || '匿名用户'; // 默认用户名// 后端返回的字段是 username
        } else {
          post.avatar = AvatarIcon; // 默认头像
          post.user_name = '匿名用户'; // 默认用户名
        }

        isEditing.value[index] = false; // 初始化编辑状态为 false
