  ...
  <div>
    ...
    <section class="cart-page flex-view">
      <div class="left-flex">
        ...
        <!-- 使用原生 HTML 的 datetime-local 类型 -->
        <input v-model="pageData.receiverTime" type="datetime-local" class="name" />
        <div class="title flex-view">
          <h3>备注</h3>
        </div>
...
import { getMemberInfoByUserIdApi } from '/@/api/thing'
...
onMounted(() => {
  ...
  if (userId) {
    getMemberInfoByUserIdApi({ userId }).then((res) => {
      if (res.code === 200) {
        pageData.receiverName = res.data.receiverName;
        pageData.receiverPhone = res.data.receiverPhone;
      } else {
        message.warn('未找到会员信息');
      }
    }).catch((err) => {
