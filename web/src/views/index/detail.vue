<template>
  ...
  <div class="top-right">
    ...
    <div class="user-info-container">
      <!-- 真实姓名 -->
      <div class="real-name">
        <div class="title-value"><span class="important">{{ detailData.title }}</span></div>
      </div>

      <!-- 会员状态和会员号 -->
      <div class="member-info">
        <div class="tag">有效</div>
        <div class="member-id">会员号：<span class="important">{{ detailData.id }}</span></div>
      </div>

      <!-- 用户头像、用户ID、用户名 -->
      <div class="user-details">
        <div class="avatar-box flex-view">
          <img v-if="uData.form && uData.form.avatar" :src="uData.form.avatar" class="avatar" />
          <img v-else :src="AvatarIcon" class="avatar" />
        </div>
        <div class="user-info">
          <div>用户ID：<span class="important">{{ detailData.userId }}</span></div>
          <div>用户名：<span class="important">{{ uData.form.username }}</span></div>
        </div>
      </div>
    </div>
    ...
    <div class="score">
      <div class="pv">
        <span class="nickname">{{ uData.form.nickname }}</span> · {{ detailData.pv }}次浏览
      </div>
    ...
    <!-- 基本信息 -->
    <div class="meta-info">
      <div class="meta">性别：<span class="important">{{ detailData.sex }}</span></div>
      <div class="meta">年龄：<span class="important">{{ detailData.age }}</span></div>
      <div class="meta">身高：<span class="important">{{ detailData.shengao }}</span></div>
      <div class="meta">地区：<span class="important">{{ detailData.address }}</span></div>
      <div class="meta">职业：<span class="important">{{ detailData.zhiye }}</span></div>
      <div class="meta">车房情况：<span class="important">{{ detailData.chefang }}</span></div>
    </div>
...
const clickRate = (num) => {
  ...
  rateApi({ thingId: thingId.value, rate: num })
    .then((res) => {
      ...
      detailData.value.rate = num; // 更新当前详情的评分
      getThingDetail();
    })
    .catch((err) => {
      console.log(err);
