...
public class OrderController {
    ...
    public APIResponse userOrderList(String myThingId, String type){
        List<Order> list =  service.getUserOrderList(myThingId, type);
        return new APIResponse(ResponeCode.SUCCESS, "查询成功", list);
    }
    ...
    @RequestMapping(value = "/yesUserOrder", method = RequestMethod.POST)
    @Transactional
    public APIResponse yesUserOrder(Long id) throws IOException {
        Order order = new Order();
        order.setId(id);
        order.setStatus("2"); // 2=同意
        service.updateOrder(order);
